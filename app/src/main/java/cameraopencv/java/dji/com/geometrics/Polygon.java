...
public class Polygon {
    ...
    public static class Builder {
        ...
        private boolean _firstPoint2D = true;
         ...
         */
        private void updateBoundingBox(Point2D point) {
            if (_firstPoint2D) {
    ...
    private boolean intersect(Line ray, Line side) {
        ...
        if (!ray.isVertical() && !side.isVertical()) {
            ...
            intersectPoint2D = new Point2D(x, y);
        ...
        // System.out.println("Intersect point: " + intersectPoint2D.toString());

        if (side.isInside(intersectPoint2D) && ray.isInside(intersectPoint2D)) {
            return true;
        }
     ...
     * @return
     */
    private Line createRay(Point2D point) {
        // create outside point
        double epsilon = (_boundingBox.xMax - _boundingBox.xMin) / 10e6;
        Point2D outsidePoint2D = new Point2D(_boundingBox.xMin - epsilon, _boundingBox.yMin);

        Line vector = new Line(outsidePoint2D, point);
        return vector;
    }
     ...
     */
    private boolean inBoundingBox(Point2D point) {
